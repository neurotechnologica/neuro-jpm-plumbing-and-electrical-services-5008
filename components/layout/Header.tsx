...
export default function Header({ content }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--color-primary)]/95 backdrop-blur-md text-white border-b border-white/10">
      {/* Desktop header */}
      <div className="hidden md:flex items-center justify-between px-8 py-3.5 max-w-7xl mx-auto">
        {/* Logo + business name */}
        ...
        <nav className="flex items-center gap-7 text-sm font-medium">
          ...
          <a
            href={`tel:${meta.phone}`}
            className="ml-2 px-5 py-2.5 rounded-xl bg-[var(--color-accent)] hover:brightness-110 transition-all font-semibold text-sm shadow-md"
          >
            {meta.phone}
          </a>
        </nav>
